...
// 开发阶段监控变化
gulp.task('dev', ['open', 'tpl'], function() {
...
gulp.task('open', ['devServer'], function() {
  // gulp.src(__filename).pipe(open({ uri: 'http://localhost:36667/index.html' }));
  gulp.src(__filename).pipe(open({ uri: 'http://localhost:36666' }));
});
